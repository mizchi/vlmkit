...
- `tools/game-asset-motion-core.mjs`: single JS owner for dogfood-only
  game-asset motion helpers that are not yet worth a package or MoonBit
  boundary: FBX/GLB/VRMA source decisions, Motion IR verification, GLB clip
  pose sampling, and runtime clip/pose playback gates.
- `tools/check-game-asset-motion-core.mjs`: dogfood smoke for that helper. It
  checks the FBX/Mixamo -> GLB -> Motion IR adapter decision, verifies the
  robot Motion IR fixture against the model, samples a roundtrip GLB clip pose,
  and runs the clip/pose playback gates without launching a browser. Available
  as `pnpm run motion:core-smoke:game-assets`.
