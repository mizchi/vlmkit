...
describe("docs/anim-ir.md", () => {
  ...
  it("has one example per kind plus the timeline", () => {
    assert.ok(blocks.length >= 7, `found ${blocks.length} json blocks`);
  });
  ...
  it("mentions every kind by heading", () => {
    for (const kind of ["sort", "state-machine", "heap", "distributed", "diagram", "vector"]) assert.match(md, new RegExp(`^## kind: ${kind}$`, "m"));
