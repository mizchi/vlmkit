...
| `scene.ts` | `SceneElement` (the `--elements` JSON), `parseSceneElements`, `sceneFromTree`, `judgeSceneIntegrity`, `sceneToCompositionInput` | image mode of `check integrity` / `check copy`; any renderer that is not a browser |
