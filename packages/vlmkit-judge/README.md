...
| `scene.ts` | `SceneElement` (the `--elements` JSON), `parseSceneElements`, `sceneFromTree`, `judgeSceneIntegrity`, `sceneToCompositionInput`, `sceneToColorRolesInput`, `sceneToDesignPolicyInput`, `judgeSceneCopy` | `--elements` mode of `check integrity` / `check copy` / `check composition` / `check color` / `check design`; any renderer that is not a browser |
...
| `copy.ts` | `analyzeCopy` — placeholder scan, manifest / forbid list, invisible-reason attribution; `parseCopyManifest`, `INVISIBLE_REASONS` | `vlmkit check copy`, on a page or a scene |
