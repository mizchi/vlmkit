...
| `scene.ts` | `SceneElement` (the `--elements` JSON), `parseSceneElements`, `sceneFromTree`, `judgeSceneIntegrity`, `sceneToCompositionInput`, `sceneToColorRolesInput`, `sceneToDesignPolicyInput` | `--elements` mode of `check integrity` / `check copy` / `check composition` / `check color` / `check design`; any renderer that is not a browser |
