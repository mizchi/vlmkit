...
| `vlmkit-anim` (separate binary, `@mizchi/vlmkit-anim`) | `check`, `validate`, `compile`, `explain`, `render`, `frames`, `sheet`, `video`, `html`, `runtime`, `eval`, `schema` — declarative explanatory animations (SVG + Web Animations): fourteen structural kinds plus `compose` (several in panes), and value / callout / snapshot / group / text annotations in every kind. Not a `vlmkit` subcommand: a standalone tool whose `eval` runs the same frame-sampled evaluator as `check animation` (package `@mizchi/vlmkit-animation-eval`); guide in [`docs/anim-ir.md`](anim-ir.md), sample GIFs under `packages/vlmkit-anim/samples/` |
