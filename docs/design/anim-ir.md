...
same tool. The `pr-visual` workflow runs `pr --mermaid` on every same-repo
pull request and keeps one comment on the PR current — a reviewer sees the
shape of a change before the diff. It used to post the GIF and contact sheet
(published on a `pr-visuals` branch); in review that read worse than one
static flowchart that renders inline, so the workflow switched to mermaid.
