...
```bash
vlmkit-anim schema --kind sort                      # the writing guide for one kind (docs/anim-ir.md has all eleven)
...
| `docs/anim-ir.md` | **Writing guide for `vlmkit-anim`**: the eleven scene kinds, the timeline layer, embedding. The one page an agent reads before producing a scene |
