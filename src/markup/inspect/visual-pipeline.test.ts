...
import { compareScreenshots, detectWhiteout } from "../../vrt/core/heatmap.ts";
import { decodePng } from "../../vrt/core/png-utils.ts";
...
async function savePng(path: string, png: { width: number; height: number; data: Uint8Array }) {
  const { encodePng } = await import("../../vrt/core/png-utils.ts");
