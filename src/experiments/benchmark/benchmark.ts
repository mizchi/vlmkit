...
async function main() {
  ...
  // ---- Computed style diff ----
  {
    const { diffComputedStyles } = await import("../css-challenge/css-challenge-core.ts");
  ...
  // ---- Paint tree diff ----
  {
    const { diffPaintTrees } = await import("@mizchi/vlmkit-capture/crater-client.ts");
  ...
  // ---- Viewport discovery ----
  {
    const { extractBreakpoints, generateViewports } = await import("@mizchi/vlmkit-capture/viewport-discovery.ts");
    const css = `
  ...
  // ---- JSONL read/write ----
  {
    const { appendRecords, readAllRecords } = await import("../detection/detection-db.ts");
