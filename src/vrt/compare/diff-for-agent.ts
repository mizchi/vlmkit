...
function formatRegionDiffEvidence(
  ...
  parts.push(`bbox ${formatRegionDiffBbox(change.bbox)}`);
  ...
  for (const [variantFile, results] of byVariant) {
    ...
    if (regionDiffSummary && (regionDiffSummary.perViewport.length > 0 || regionDiffSkippedViewports.length > 0)) {
      ...
      if (changeRows.length > 0) {
        const authoredLookup = buildAuthoredStyleValueLookup(report, variantFile);
        lines.push("| Viewport | Selector | Property | Target | Current | Evidence | Artifacts |");
        lines.push("|---|---|---|---|---|---|---|");
        for (const { entry, change } of changeRows.slice(0, 12)) {
          ...
          const artifacts = formatRegionDiffArtifacts(entry) || "-";
          lines.push(
            `| \`${entry.viewport}\` | ${selector} | \`${change.property}\` | ${target} | ${current} | ${evidence} | ${artifacts} |`,
          );
        }
      ...
      if (summaries.length > 0) {
        lines.push(`Notes: ${summaries.join(" / ")}`);
        lines.push("");
      }
    }
