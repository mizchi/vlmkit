  ...
  for (const [variantFile, results] of byVariant) {
    ...
    if (regionDiffSummary && regionDiffSummary.perViewport.length > 0) {
      ...
      lines.push("Optional VLM handoff generated by `migration compare --region-diff`. " +
        ...
        "CSS property, sampled before/after colors, and the full JSON/Markdown artifacts.");
      lines.push("");
      ...
      if (changeRows.length > 0) {
        ...
        for (const { entry, change } of changeRows.slice(0, 12)) {
          const selector = formatNullableCode(change.selector ?? change.selectorHint);
          const value = `${formatNullableCode(change.from)} → ${formatNullableCode(change.to)}`;
