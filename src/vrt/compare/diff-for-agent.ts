...
export interface DfaRegionDiffChange {
...
export interface DfaRegionDiffSkippedViewport {
  viewport: string;
  diffRatio: number;
  diffPixels: number;
...
export interface DfaRegionDiffSummary {
  variantFile: string;
  maxViewports?: number;
  perViewport: Array<{
    viewport: string;
    ...
    summary?: string;
    changeCount: number;
    changes: DfaRegionDiffChange[];
    error?: string;
  }>;
  skippedViewports?: DfaRegionDiffSkippedViewport[];
}
...
export interface DfaReport {
  ...
  regionDiffs?: DfaRegionDiffSummary[];
  stateDiffs?: Array<{
    variantFile: string;
    perState: Array<{
      state: string;
...
function formatNullableCode(value: string | number | null | undefined): string {
  return value === null || value === undefined || value === "" ? "-" : `\`${value}\``;
}

function formatRegionDiffBbox(bbox: DfaRegionDiffChange["bbox"]): string {
  return bbox ? `\`${bbox.left},${bbox.top} ${bbox.width}x${bbox.height}\`` : "-";
}

function formatRegionDiffArtifacts(entry: DfaRegionDiffSummary["perViewport"][number]): string {
  const parts: string[] = [];
  if (entry.jsonPath) parts.push(`json: \`${entry.jsonPath}\``);
  if (entry.markdownPath) parts.push(`md: \`${entry.markdownPath}\``);
  return parts.join("; ");
}

function unscopedAuthoredSelector(selector: string): string {
  const sep = " :: ";
  const sepIndex = selector.indexOf(sep);
  if (selector.startsWith("@media ") && sepIndex >= 0) {
    return selector.slice(sepIndex + sep.length).trim();
  }
  return selector;
}

function buildAuthoredStyleValueLookup(report: DfaReport, variantFile: string): Map<string, DfaCsdEntry> {
  const lookup = new Map<string, DfaCsdEntry>();
  const authored = (report.authoredStyleDiff ?? []).find((entry) => entry.variantFile === variantFile);
  for (const entry of authored?.result.entries ?? []) {
    const selector = unscopedAuthoredSelector(entry.selector);
    const key = `${selector} ${entry.property}`;
    if (!lookup.has(key)) lookup.set(key, entry);
  }
  return lookup;
}

function formatRegionDiffValue(
  change: DfaRegionDiffChange,
  authored: DfaCsdEntry | undefined,
): string {
  const sampled = `${formatNullableCode(change.from)} → ${formatNullableCode(change.to)}`;
  if (!authored) return sampled;
  const corrected = `${formatNullableCode(authored.baseline)} → ${formatNullableCode(authored.variant)}`;
  if (authored.baseline === change.from && authored.variant === change.to) {
    return `${corrected} _(authored CSSOM)_`;
  }
  return `${corrected} _(authored CSSOM; sampled ${sampled})_`;
}

function regionDiffConfidenceRank(confidence: DfaRegionDiffChange["confidence"] | undefined): number {
...
function selectRegionDiffNextStep(
  report: DfaReport,
  variantFile: string,
  preferredViewport: string | undefined,
): {
  viewport: string;
  selector: string;
  property: string;
  target: string;
  current: string | null;
  valueSource: "authored CSSOM" | "sampled";
  artifacts: string;
} | null {
  ...
  return {
    ...
    property: best.change.property,
    target: best.authored?.baseline ?? best.change.from!,
    current: best.authored?.variant ?? best.change.to,
    valueSource: best.authored ? "authored CSSOM" : "sampled",
...
export function formatMigrationReportForAgent(
  ...
  for (const [variantFile, results] of byVariant) {
    ...
    const regionDiffSummary = (report.regionDiffs ?? []).find((r) => r.variantFile === variantFile);
    const regionDiffSkippedViewports = regionDiffSummary?.skippedViewports ?? [];
    if (regionDiffSummary && (regionDiffSummary.perViewport.length > 0 || regionDiffSkippedViewports.length > 0)) {
      lines.push("### VLM region diff (selector/property handoff)");
      lines.push("");
      lines.push("Optional VLM handoff generated by `migration compare --region-diff`. " +
        "Use this before guessing from raw pixels: it points at the likely selector, " +
        "CSS property, authored before/after values when available, sampled colors, " +
        "and the full JSON/Markdown artifacts.");
      lines.push("");
      ...
      const errorRows = regionDiffSummary.perViewport.filter((entry) => entry.error);
      if (errorRows.length > 0) {
        lines.push("| Viewport | Error |");
        lines.push("|---|---|");
        for (const entry of errorRows) {
          lines.push(`| \`${entry.viewport}\` | ${entry.error} |`);
        }
        lines.push("");
      }

      const changeRows = regionDiffSummary.perViewport.flatMap((entry) =>
        entry.changes.map((change) => ({ entry, change })),
      );
      if (changeRows.length > 0) {
        const authoredLookup = buildAuthoredStyleValueLookup(report, variantFile);
        lines.push("| Viewport | Selector | Property | Baseline → Variant | Δ | Bbox | Confidence |");
        lines.push("|---|---|---|---|---:|---|---|");
        for (const { entry, change } of changeRows.slice(0, 12)) {
          ...
          const authored = selectorText
            ...
            : undefined;
          const value = formatRegionDiffValue(change, authored);
          const delta = change.averageChannelDelta === null ? "-" : String(change.averageChannelDelta);
          const confidence = change.selectorConfidence
            ? `${change.confidence} / ${change.selectorConfidence}`
            : change.confidence;
          lines.push(
            `| \`${entry.viewport}\` | ${selector} | \`${change.property}\` | ${value} | ${delta} | ` +
            `${formatRegionDiffBbox(change.bbox)} | ${confidence} |`,
          );
        }
        if (changeRows.length > 12) {
          lines.push(`| _…${changeRows.length - 12} more region changes_ | | | | | | |`);
        }
        lines.push("");
      }

      const summaries = regionDiffSummary.perViewport
        .filter((entry) => entry.summary)
        .map((entry) => `\`${entry.viewport}\`: ${entry.summary}`);
      if (summaries.length > 0) {
        lines.push(`Summary: ${summaries.join(" / ")}`);
        lines.push("");
      }

      const artifactRows = regionDiffSummary.perViewport
        .map((entry) => ({ viewport: entry.viewport, artifacts: formatRegionDiffArtifacts(entry) }))
        .filter((entry) => entry.artifacts);
      if (artifactRows.length > 0) {
        lines.push("Artifacts:");
        for (const entry of artifactRows) {
          lines.push(`- \`${entry.viewport}\`: ${entry.artifacts}`);
        }
        lines.push("");
      }
    }
    ...
    const regionDiffNextStep = selectRegionDiffNextStep(report, variantFile, sorted[0]?.viewport);
