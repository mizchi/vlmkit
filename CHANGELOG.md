...
- **Two smoke harnesses had been failing every command since the 0.6 rename.** Chasing the
  ...
  **Deliberately left**, because each names something real rather than the tool:
  ...
  block. Still open: ~74 *bare* flat command names (`media-variants`, `component-from-image`)
  in `Spec.pkl` / `docs/SPEC.md` prose. That is the adjacent command-grouping rename, not this
  one, and it needs per-site classification — several are rule names, module names or
  output-directory defaults rather than commands. `docs/SPEC.md` is generated by `pkspec`,
  which is unavailable here, so it was updated in lockstep with `Spec.pkl` by hand.
